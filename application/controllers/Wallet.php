...
class Wallet extends CI_Controller {
	...
	public function index(){
		...
		$address = $this->db->select('address, btc, unconfirmed')->where('btc', 0)->where('unconfirmed', 0)->where('user_id', $user->id)->get('address')->row();
		if (!$address) {
			$data = [
		    	'user_id' => $user->id,
		    	'address' => $address = $this->core->address()
		...
		$user->transactions = $this->db->where('user_id', $user->id)->get('wisecoin_extrato')->result();
	...
	public function transactions(){
		...
		$user = $this->user->check($this->input->get_request_header('Authorization'));
		$data = $this->db->select('id, cota, data, wsc, descricao')->where('user_id', $user->id)->order_by('id', 'desc')->get('wisecoin_extrato')->result();
